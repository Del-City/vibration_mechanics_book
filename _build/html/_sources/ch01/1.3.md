...
$$
\begin{aligned}
x_{\max} &= x_{\mathrm{st}} + X_{\mathrm{dyn}} = \frac{mgL^3}{48EJ} + \beta\frac{PL^3}{48EJ} \\
...
方程{eq}`eq:1-68`的通解仍是对应齐次方程的通解和非齐次方程的特解之和。前者与式{eq}`eq:1-36`或式{eq}`eq:1-39`完全相同。假定干扰力 $F_{\mathrm{e}}(t)=P\sin\theta t$, 则考虑阻尼作用的单自由度系统运动方程为[以方程{eq}`eq:1-68b`为例]
...
```{note}
【例2-7】某部件在液体中运动, 干扰力 $P=49\sin\theta t\ (\mathrm{N})$, 在周期 $\tau=0.2\ \mathrm{s}$ 时发生共振, 振幅为 $0.50\ \mathrm{cm}$, 求阻尼系数 $c$。
...
当 $\theta=\omega$, 即 $r=1$ 时, 由式\eqref{eq:1-82}和式\eqref{eq:1-83}得
...
注意到 $A_{1}=A_{2}=0.707A_{0}$, 再由式①和式②, 可知与 $A_{1}$、$A_{2}$ 所对应的 $r$ 值应满足方程
